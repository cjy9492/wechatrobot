		...
		for (int i = 0; i < msgList.size(); i++) {
			...
			if (m.getString("FromUserName").contains("@@") || m.getString("ToUserName").contains("@@")) { // 群聊消息
				...
				if (m.getString("Content").contains("<br/>")) {
					...
					String groupSendId = m.getString("Content").substring(0,m.getString("Content").lastIndexOf(":<br/>"));
					m.put("groupSendId",groupSendId);
			...
			LOG.info("消息类型是:"+m.getInteger("MsgType"));
		...
		while (true) {
			if (core.getMsgList().size() > 0 && core.getMsgList().get(0).getString("Content") != null) {
				if (core.getMsgList().get(0).getString("Content").length() > 0) {
					...
					System.out.println("消息类型为:"+msg.getString("Type") );
